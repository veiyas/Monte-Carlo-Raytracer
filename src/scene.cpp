...
Scene::Scene()
{
	...
	_spheres.emplace_back(1.f, Color{ 0.1, 0.1, 1.0 }, Vertex{ 7.f, -3.f, -3.f, 1.f }, 1.f);
	_spheres.emplace_back(1.f, Color{ 0.1, 0.1, 1.0 }, Vertex{ 7.f, 3.f, -3.f, 1.f }, 1.f);
	...
	//Lights
	_pointLights.emplace_back(Vertex(5, 4, 4, 1), Color(1, 1, 1));
}
...
Color Scene::intersection(Ray& ray)
{
	...
	for (auto& triangle : _sceneTris)
	{
		...
		if (t != -1 && t < minT)
		{
			closestIntersectPoint = ray.getStart() + (ray.getEnd() - ray.getStart()).operator*=(t);
	...
	for (auto& tetrahedron : _tetrahedrons)
	{
		...
		{
			closestIntersectPoint = ray.getStart() + (ray.getEnd() - ray.getStart()).operator*=(intersect.first);
	...
	closestIntersectColor *= shadowRayContribution(closestIntersectPoint, closestIntersectNormal);
...
{
	//THIS WORKS BUT NEEDS TO WORK ON A POINT BASIS, NOT TRIANGLE BASIS
